...
////Intel SDK
//SLR Run testing submitting NDRange kernel with 10 groups but a single kernel without group specification attributes.
#include <iostream>
#include <cstdio>
#include <ctime>
#include <assert.h>
...
int main(int argc, char **argv) {
  ...
  printf("SLR Run: 1 Kernel, 1 Queue, 1 Thread, 10 Groups");
...
bool init_opencl() {
  ...
  std::string binary_file = getBoardBinaryFile("slr_g", device[0]);
  printf("Using AOCX: %s\n", binary_file.c_str());
