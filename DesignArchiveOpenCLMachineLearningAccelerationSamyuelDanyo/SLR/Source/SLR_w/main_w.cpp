...
////Intel SDK
//Testing with transfer of weights
#include <iostream>
#include <cstdio>
#include <ctime>
#include <assert.h>
...
int main(int argc, char **argv) {
  ...
  const char* kernel_name = "simple_linear_regression_w";
  acl_thread_t t1;
  cl_int status;
  ...
  printf("SLR Run: 1 Kernel, 1 Queues, 1 Thread, Transfer of Weights");
...
bool init_opencl() {
  ...
  std::string binary_file = getBoardBinaryFile("slr_w", device[0]);
  printf("Using AOCX: %s\n", binary_file.c_str());
...
void Simple_Linear_Regression::init() {
  ...
  for(unsigned i = 0; i < num_devices; ++i) {
    ...
    weights[i].reset(n_per_device[i]);
    ...
    weights[i] = (float *)alignedMalloc(sizeof(float) * n_per_device[i]);
    ...
    for(unsigned j = 0; j < n_per_device[i]; ++j) {
      ...
      if(!strcmp(kernel_name,"simple_linear_regression_w")) {
...
//!!!!!!!!LINES UP TO VERIFICATION NEED TO BE INDENTED 2 SPACES!!!!!!
  ...
  for(unsigned i = 0; i < num_devices; ++i) {
    ...
    status = clEnqueueWriteBuffer(queue[i], weights_buf[i], CL_FALSE,
        0, n_per_device[i] * sizeof(float), weights[i], 0, NULL, &write_event[1]);
    checkError(status, "Failed to transfer the input X dataset");
