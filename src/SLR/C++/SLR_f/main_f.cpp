...
public:
  ...
  unsigned I;	//Number of run iterations
  ...
  scoped_array<cl_mem> features_buf;

  // Problem data.
  scoped_array<scoped_aligned_ptr<float> > input_X;
  scoped_array<scoped_aligned_ptr<float> > prediction_Y;
  scoped_array<scoped_aligned_ptr<float> > weights;
  scoped_array<scoped_aligned_ptr<float> > features;
...
int main(int argc, char **argv) {
  ...
  const char* kernel_name = "simple_linear_regression_f";
  acl_thread_t t1;
  cl_int status;
  ...
  printf("SLR Run: 1 Kernel, 1 Queue, 1 Thread, Transfer of Weights & Features");
...
void* start_helper(void * arg) {
  ...
  printf("Instantiating a new simple linear regression inference with args: N=%d, I=%d in [%f:%f],kernel_name=%s,output_file=%s \n",slr1->N,slr1->I,slr1->S,slr1->E,slr1->kernel_name,slr1->OUT_F_NAME);
  slr1->start();
  return NULL;
}
...
bool init_opencl() {
  ...
  std::string binary_file = getBoardBinaryFile("slr_f", device[0]);
  printf("Using AOCX: %s\n", binary_file.c_str());
...
void Simple_Linear_Regression::init() {
  ...
  features_buf.reset(num_devices);
  ...
  for(unsigned i = 0; i < num_devices; ++i) {
    ...
    // Input buffers
    weights_buf[i] = clCreateBuffer(context, CL_MEM_READ_WRITE,
        4 * sizeof(float), NULL, &status);
    checkError(status, "Failed to create buffer for output");
 
    features_buf[i] = clCreateBuffer(context, CL_MEM_READ_WRITE,
         n_per_device[i] * 4 * sizeof(float), NULL, &status);
    checkError(status, "Failed to create buffer for output");
 }

  //initializing the data and calculating the reference output for comparison.
  input_X.reset(num_devices);
  weights.reset(num_devices);
  features.reset(num_devices);
  ...
  for(unsigned i = 0; i < num_devices; ++i) {
    ...
    weights[i].reset(4);
    features[i].reset(n_per_device[i] * 4);
    ...
    weights[i] = (float *)alignedMalloc(sizeof(float) * 4);
    features[i] = (float *)alignedMalloc(sizeof(float) * n_per_device[i] * 4);
    ...
    for(unsigned j = 0; j < n_per_device[i]; ++j) {
      ...
      if(!strcmp(kernel_name,"simple_linear_regression_f")) {
...
void Simple_Linear_Regression::run() {
  ...
  for(unsigned i = 0; i < I; ++i) {
    ...
    for(unsigned i = 0; i < num_devices; ++i) {
      ...
      if(!strcmp(kernel_name,"simple_linear_regression_f")) {
        weights[i][0] = -0.55395846;
        weights[i][1] = 1;
        weights[i][2] = 10; 
        weights[i][3] = 8.94836441;
       

        for(unsigned j = 0; j < n_per_device[i]; ++j) {
	  features[i][4 * j] = 1;
          features[i][4 * j + 1] = input_X[i][j];
          features[i][4 * j + 2] = sin(3 * input_X[i][j]);
          features[i][4 * j + 3] = exp(-1 * input_X[i][j] * input_X[i][j]); 
	}
      }  

      cl_event write_event[2];

      status = clEnqueueWriteBuffer(queue[i], weights_buf[i], CL_FALSE,
        0, 4 * sizeof(float), weights[i], 0, NULL, &write_event[0]);
      checkError(status, "Failed to transfer the input X dataset");
    
      status = clEnqueueWriteBuffer(queue[i], features_buf[i], CL_FALSE,
        0, 4 * n_per_device[i] * sizeof(float), features[i], 0, NULL, &write_event[1]);
      checkError(status, "Failed to transfer the input X dataset");
    
      // Set kernel arguments.
      unsigned argi = 0;

      status = clSetKernelArg(kernel[i], argi++, sizeof(cl_mem), &weights_buf[i]);
      checkError(status, "Failed to set argument %d", argi - 1);

      status = clSetKernelArg(kernel[i], argi++, sizeof(cl_mem), &features_buf[i]);
      checkError(status, "Failed to set argument %d", argi - 1);
...
void Simple_Linear_Regression::cleanup() {
  for(unsigned i = 0; i < num_devices; ++i) {
    ...
    if(features_buf && features_buf[i]) {
      clReleaseMemObject(features_buf[i]);
    }
