...
////Intel SDK
//SLR Run with 10 groups and specified attribute for required 10000 work group size in kernel.
#include <iostream>
#include <cstdio>
#include <ctime>
#include <assert.h>
...
int main(int argc, char **argv) {
  ...
  printf("SLR Run: 1 kernel + work size attribute(10000, 1 queue, 1 thread, 10 work groups");
...
bool init_opencl() {
  ...
  std::string binary_file = getBoardBinaryFile("slr_k_g", device[0]);
  printf("Using AOCX: %s\n", binary_file.c_str());
...
//!!!!LINES UP TO VERIFICATION  NEED TO BE IDNENTED 2 SPACES BECAUSE OF THE ITERATIONS FOR!!!!!!!!!
...
//    const size_t local_work_size = global_work_size / 10;
	const size_t local_work_size = 10000;
    printf("\n\nLaunching for device %d (%d elements),[%d per Work Group]", i, global_work_size, local_work_size);
