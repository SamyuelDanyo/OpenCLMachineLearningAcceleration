...
////Intel SDK
//SLR Run 2 kernels, 2 queues, 1 thread
#include <iostream>
#include <cstdio>
#include <ctime>
#include <assert.h>
...
public:
  ...
  Simple_Linear_Regression(unsigned _N,unsigned _I,float _S,float _E,const char* _OUT_F_NAME,const char* _LOG_F_NAME,const char* _kernel_name,const char* _kernel_name_1):N(_N),I(_I),S(_S),E(_E),OUT_F_NAME(_OUT_F_NAME),LOG_F_NAME(_LOG_F_NAME),kernel_name(_kernel_name),kernel_name_1(_kernel_name_1){
  }

  scoped_array<cl_command_queue> queue;
  scoped_array<cl_command_queue> queue_1;
  scoped_array<cl_kernel> kernel;
  scoped_array<cl_mem> input_X_buf;
  scoped_array<cl_mem> prediction_Y_buf;
  scoped_array<cl_kernel> kernel_1;
  scoped_array<cl_mem> input_X_buf_1;
  scoped_array<cl_mem> prediction_Y_buf_1;
  ...
  scoped_array<scoped_aligned_ptr<float> > input_X_1;
  scoped_array<scoped_aligned_ptr<float> > prediction_Y_1;
  scoped_array<scoped_array<float> > ref_output_calc_1;
  scoped_array<scoped_array<float> > ref_output_pred_1; 
  scoped_array<unsigned> n_per_device;
  scoped_array<unsigned> n_per_queue;
  void init();
  void run();
  void cleanup();
  void start();
...
int main(int argc, char **argv) {
  ...
  const char* kernel_name_1 = "simple_linear_regression_1";
  acl_thread_t t1;
  cl_int status;
  ...
  if(options.has("log_f_name")) {
   ...
   printf("SLR Run: 2 Kernel, 2 Queues, 1 Thread");
  ...
  Simple_Linear_Regression slr1(n,i,s,e,f_name,log_f_name,kernel_name,kernel_name_1);
...
bool init_opencl() {
  ...
  std::string binary_file = getBoardBinaryFile("slr_k_q", device[0]);

  printf("Using AOCX: %s, %s\n", binary_file.c_str());
...
void Simple_Linear_Regression::init() {
  ...
  kernel_1.reset(num_devices);
  n_per_device.reset(num_devices);
  n_per_queue.reset(num_devices);
  input_X_buf.reset(num_devices);
  input_X_buf_1.reset(num_devices);
  prediction_Y_buf.reset(num_devices);
  prediction_Y_buf_1.reset(num_devices);
  ...
  for(unsigned i = 0; i < num_devices; ++i) {
    ...
    kernel_1[i] = clCreateKernel(program, kernel_name_1, &status);
    checkError(status, "Failed to create kernel_1");

    // Determine the number of elements processed by this device.
    n_per_device[i] = N / num_devices; // number of elements handled by this device
    n_per_queue[i] = n_per_device[i] / 2;
    ...
    if(i < (n_per_device[i] % 2)) {
      n_per_queue[i]++;
    }

    // Input buffer.
    input_X_buf[i] = clCreateBuffer(context, CL_MEM_READ_WRITE,
        n_per_queue[i] * sizeof(float), NULL, &status);
    checkError(status, "Failed to create buffer for input A");

    input_X_buf_1[i] = clCreateBuffer(context, CL_MEM_READ_WRITE,
        n_per_queue[i] * sizeof(float), NULL, &status);
    checkError(status, "Failed to create buffer for input A");
    ...
    prediction_Y_buf_1[i] = clCreateBuffer(context, CL_MEM_READ_WRITE,
        n_per_queue[i] * sizeof(float), NULL, &status);
    checkError(status, "Failed to create buffer for output");
  ...
  input_X_1.reset(num_devices);
  prediction_Y_1.reset(num_devices);
  ref_output_calc_1.reset(num_devices);
  ref_output_pred_1.reset(num_devices);
  ...
  // aligned buffer. 
  for(unsigned i = 0; i < num_devices; ++i) {
    input_X[i].reset(n_per_queue[i]);
    prediction_Y[i].reset(n_per_queue[i]);
    ref_output_calc[i].reset(n_per_queue[i]);
    ref_output_pred[i].reset(n_per_queue[i]);
    input_X[i] = (float *)alignedMalloc(sizeof(float) * n_per_queue[i]);
    prediction_Y[i] = (float *)alignedMalloc(sizeof(float) * n_per_queue[i]);

    input_X_1[i].reset(n_per_queue[i]);
    prediction_Y_1[i].reset(n_per_queue[i]);
    ref_output_calc_1[i].reset(n_per_queue[i]);
    ref_output_pred_1[i].reset(n_per_queue[i]);
    input_X_1[i] = (float *)alignedMalloc(sizeof(float) * n_per_queue[i]);
    prediction_Y_1[i] = (float *)alignedMalloc(sizeof(float) * n_per_queue[i]);

    for(unsigned j = 0; j < n_per_queue[i]; ++j) {
      input_X[i][j] = rand_float(S,E);
      // Calculate the refrence output based on the current kernel.
      if(!strcmp(kernel_name,"simple_linear_regression")) {
...
//        ref_output_pred[i][j] = cos ( 2 * acos (input_X[i][j]) );
    ...
    for(unsigned j = 0; j < n_per_queue[i]; ++j) {
      input_X_1[i][j] = rand_float(S,E);
      // Calculate the refrence output based on the current kernel.
      if(!strcmp(kernel_name_1,"simple_linear_regression_1")) {
//          ref_output_calc_1[i][j] = -0.99489651 + input_X_1[i][j] * 0.0170479 + input_X_1[i][j] * input_X_1[i][j] * 2.01794626;
            ref_output_calc_1[i][j] = -0.55395846  + input_X_1[i][j] * 1 + sin ( 3 * input_X_1[i][j]) * 10 + exp ( -input_X_1[i][j] * input_X_1[i][j]) * 8.94836441;
//          ref_output_pred[i][j] = cos ( 2 * acos (input_X[i][j]) );
            ref_output_pred_1[i][j] = 10 * exp( -2 * input_X_1[i][j] * input_X_1[i][j] ) + sin( 3 * input_X_1[i][j] ) * 10 + input_X_1[i][j];
      //
   }
  }
 }
...
//!!!!!LINES UP TO VERIFICATION NEED TO BE INDENTED 2 SPACES
  ...
  scoped_array<cl_event> kernel_event_1(num_devices);
  scoped_array<cl_event> finish_event_1(num_devices);
  ...
  for(unsigned i = 0; i < num_devices; ++i) {
    ...
    status = clEnqueueWriteBuffer(queue[i], input_X_buf[i], CL_FALSE,
        0, n_per_queue[i] * sizeof(float), input_X[i], 0, NULL, &write_event[0]);
    checkError(status, "Failed to transfer the input X dataset");

    status = clEnqueueWriteBuffer(queue_1[i], input_X_buf_1[i], CL_FALSE,
        0, n_per_queue[i] * sizeof(float), input_X_1[i], 0, NULL, &write_event[1]);
    checkError(status, "Failed to transfer the input X_1 dataset");
    ...
    status = clSetKernelArg(kernel_1[i], argi_1++, sizeof(cl_mem), &input_X_buf_1[i]);
    checkError(status, "Failed to set argument %d", argi_1 - 1);

    status = clSetKernelArg(kernel_1[i], argi_1++, sizeof(cl_mem), &prediction_Y_buf_1[i]);
    checkError(status, "Failed to set argument %d", argi_1 - 1);
    ...
    const size_t global_work_size = n_per_queue[i];
    printf("\n\nLaunching for device %d (%d elements per queue)", i, global_work_size);
    
    status = clEnqueueNDRangeKernel(queue[i], kernel[i], 1, NULL,
        &global_work_size, NULL, 1, write_event, &kernel_event[i]);
    checkError(status, "Failed to launch kernel");

    status = clEnqueueNDRangeKernel(queue_1[i], kernel_1[i], 1, NULL,
        &global_work_size, NULL, 1, write_event, &kernel_event_1[i]);
    checkError(status, "Failed to launch kernel_1");

    // Read the result. This the final operation.
    status = clEnqueueReadBuffer(queue[i], prediction_Y_buf[i], CL_FALSE,
        0, n_per_queue[i] * sizeof(float), prediction_Y[i], 1, &kernel_event[i], &finish_event[i]);
    checkError(status, "Failed to read buffer");
   
    status = clEnqueueReadBuffer(queue_1[i], prediction_Y_buf_1[i], CL_FALSE,
        0, n_per_queue[i] * sizeof(float), prediction_Y_1[i], 1, &kernel_event_1[i], &finish_event_1[i]);
    checkError(status, "Failed to read buffer");
  ...
  // Wait for all devices to finish.
   ...
   clWaitForEvents(num_devices, finish_event_1);
  ...
  for(unsigned i = 0; i < num_devices; ++i) {
    ...
    printf("\nKernel time (device %d): %0.3f ms\n", i, double(time_ns) * 1e-6);
 }

  for(unsigned i = 0; i < num_devices; ++i) {
    cl_ulong time_ns = getStartEndTime(kernel_event_1[i]);
    printf("Kernel_1 time (device %d): %0.3f ms\n", i, double(time_ns) * 1e-6);
 }
  ...
  for(unsigned i = 0; i < num_devices; ++i) {
    ...
    clReleaseEvent(kernel_event_1[i]);
    clReleaseEvent(finish_event_1[i]);
  }
  }
  // Verify results.
  pass_calc = true;
  ...
  for(unsigned i = 0; i < num_devices; ++i) {
    ...
    for(unsigned j = 0; j < n_per_queue[i]; ++j) {
      if(fabsf(ref_output_calc_1[i][j] - prediction_Y_1[i][j]) > fabsf(0.005 * ref_output_calc_1[i][j])) {
        fprintf(file0, "Failed calculation verification of device %d, index %d\nInput: %f\nOutput: %f\nReference: %f\n", i, (n_per_queue + j), input_X_1[i][j],prediction_Y_1[i][j], ref_output_calc_1[i][j]);
        pass_calc = false;
        }
      if(fabsf(ref_output_pred_1[i][j] - prediction_Y_1[i][j]) > (fabsf(0.05 * ref_output_pred_1[i][j]) + 0.03 * 25)) {
        fprintf(file0, "Failed prediction verification of device %d, index %d\nInput: %f\nOutput: %f\nReference: %f\n", i, (n_per_queue + j), input_X_1[i][j], prediction_Y_1[i][j], ref_output_pred_1[i][j]);
        pass_pred = false;
      } 
   }
  }
  ...
  for(unsigned j = 0; j < n_per_queue[i]; ++j) {
      fprintf(file1, "%d\t%f\t%f\t%f\t%f\t%f\t%f\n", (n_per_queue + j + i * n_per_device[i-1]),input_X_1[i][j],ref_output_pred_1[i][j],ref_output_calc_1[i][j],prediction_Y_1[i][j],(ref_output_calc_1[i][j]-prediction_Y_1[i][j]),(ref_output_pred_1[i][j]-prediction_Y_1[i][j]));
   }
...
void Simple_Linear_Regression::cleanup() {
  for(unsigned i = 0; i < num_devices; ++i) {
    ...
    if(kernel_1 && kernel_1[i]) {
      clReleaseKernel(kernel_1[i]);
    }
    if(queue_1 && queue_1[i]) {
      clReleaseCommandQueue(queue_1[i]);
    }
    if(input_X_buf_1 && input_X_buf_1[i]) {
      clReleaseMemObject(input_X_buf_1[i]);
    }
    if(prediction_Y_buf_1 && prediction_Y_buf_1[i]) {
      clReleaseMemObject(prediction_Y_buf_1[i]);
    }
 }
}
