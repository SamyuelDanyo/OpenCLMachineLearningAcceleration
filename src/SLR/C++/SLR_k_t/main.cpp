...
int main(int argc, char **argv) {
  ...
  if(options.has("log_f_name")) {
   ...
   printf("SLR Run: 2 Kernel, 2 Threads");
  ...
  Simple_Linear_Regression slr2(n / 2,i,s,e,"data_1.dat","log_1.log",kernel_name_1);
...
bool init_opencl() {
  ...
  std::string binary_file = getBoardBinaryFile("slr_k_t", device[0]);
  printf("Using AOCX: %s\n", binary_file.c_str());
